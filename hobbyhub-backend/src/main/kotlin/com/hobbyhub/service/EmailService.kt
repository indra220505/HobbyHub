...
class EmailService(
    ...
    fun sendOtpEmail(toEmail: String, otpCode: String) {
        ...
        // Always log OTP immediately so developer/admin can see it instantly
        log.info("==========================================================================")
        log.info("VERIFICATION OTP CODE FOR [{}]: [{}]", cleanToEmail, otpCode)
        log.info("==========================================================================")

        // Dispatch mail in non-blocking background thread to prevent HTTP delay/502
        CompletableFuture.runAsync {
            // Check if SMTP password is placeholder
            if (mailPassword == "none" || mailPassword.isBlank()) {
                log.warn("⚠️ SPRING_MAIL_PASSWORD is not set or placeholder 'none'. Skipping SMTP dispatch.")
                return@runAsync
            }
            ...
            try {
                ...
                log.error("❌ SMTP DISPATCH ERROR for [{}]: {}", cleanToEmail, e.message)
                log.warn("👉 Hint: Ensure SPRING_MAIL_PASSWORD is a 16-character Gmail App Password (not your regular password).")
            }
        }
    }
}
